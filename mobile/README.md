...
| Gate Pass lookup / Transfer / Receive (Facility) | `GET /MaterialMovement/by-gate-pass/{gatePassNumber}`, `POST /MaterialMovement/{id}/{transfer,receive}` | Reused, unchanged — built for this app in an earlier phase, wired up here for the first time (see "Gate Pass (Facility)" below) |
| Add Asset → link to a Purchase Requisition | `GET /PurchaseRequisition/available-lines?prNumber=`, `GET /PurchaseRequisition/{id}/qr` | Reused, unchanged — `prNumber` filter and the QR endpoint are new but additive on an already-existing endpoint (see "Link to a Purchase Requisition" below) |
| Add Asset → rental vendor picker | `GET /Vendor` | Reused, unchanged |
...
### Gate Pass (Facility) — zero backend change

Lets a **Facility** (or Super Admin/IT Admin) user scan the QR code
printed on a Material Movement gate pass — generated by the main web app
the instant a movement clears final approval — and confirm the physical
**Transfer (Outward)** or **Receive (Inward)** step from their phone,
instead of that being a web-only action. Every endpoint this calls
(`GET /MaterialMovement/by-gate-pass/{gatePassNumber}`,
`POST /MaterialMovement/{id}/transfer`, `POST /MaterialMovement/{id}/receive`)
already existed server-side, built in an earlier phase of this project
specifically for this app — this is the first time anything in this repo
actually calls them.

The QR itself encodes nothing but the bare `GatePassNumber` (e.g.
`GP-2026-000123`) — same content-only convention `parseQRCode()` already
handles for asset tags, so no changes were needed there. A dedicated
`app/(app)/gate-pass/scan.tsx` scanner (separate from the asset scanner —
`scan.tsx` stays untouched) hands the scanned code straight to
`app/(app)/gate-pass/[gatePassNumber].tsx`, which looks the movement up,
shows its from/to/items/gate-pass details, and offers exactly one action
button, chosen by the movement's current status: `AwaitingTransfer` →
Confirm Transfer, `Dispatched` → Confirm Receipt (with an optional
discrepancy-notes field), anything else → read-only.

No offline queueing here — like ordinary asset Transfer, both actions
need a live server decision, so this screen requires connectivity the
same way `asset/[id]/transfer.tsx` already does.

Gating: a new `canHandleGatePass()` helper in `src/lib/auth-context.tsx`
(Facility/Super Admin/IT Admin — matching
`MaterialMovementController`'s `[Authorize(Roles = "Facility,Super
Admin,IT Admin")]` exactly) is a separate permission from
`canManageAssets()`, not an extension of it — Facility does not gain Add
Asset access, and Super Admin/IT Admin keep the same override they
already have on every other privileged action in this app.

Files: `app/(app)/gate-pass/scan.tsx`, `app/(app)/gate-pass/[gatePassNumber].tsx`,
`src/api/materialMovements.ts`, `canHandleGatePass()` in
`src/lib/auth-context.tsx`, the `Facility` addition to `AppRole` and the
`MaterialMovementResponse`/`ReceiveMaterialMovementRequest` types in
`src/types/api.ts`.
...
Gate Pass Transfer/Receive uses a third, separate gate,
`canHandleGatePass()` (`Facility`/`Super Admin`/`IT Admin`) — deliberately
not folded into `canManageAssets()`, since Facility should see the Gate
Pass action but not Add Asset/Transfer/Audit, and vice versa for
ordinary Super Admin/IT Admin users. `MaterialMovementController`'s
`[Authorize(Roles = "Facility,Super Admin,IT Admin")]` is the real,
server-side enforcement here too — same "UI convenience, not the
boundary" caveat as above.
...
mobile/
  app/                      Expo Router screens (file-based routing)
    ...
    (app)/                  Everything behind the auth guard
      ...
      asset/scan-pr.tsx      Scan a PR/PO QR from within Add Asset
      audit/index.tsx       Start / browse audit sessions
      audit/[id].tsx        Active session: scan loop, results, complete
      gate-pass/scan.tsx    Scan a gate pass QR (Facility)
      gate-pass/[gatePassNumber].tsx  Transfer / Receive
  src/
...
10. **Gate Pass (Facility)** — as a Facility (or Super Admin/IT Admin)
    test user, scan a real `AwaitingTransfer` gate pass QR (print one
    from the web app by approving a Material Movement through to final
    approval); confirm the movement's details render and "Confirm
    Transfer (Outward)" appears; tap it and confirm the status flips to
    `Dispatched` in both the app and the existing web Material Movement
    screen. Scan the same QR again (or as a different Facility user at
    the destination) and confirm "Confirm Receipt (Inward)" appears
    instead; tap it and confirm the status flips to `Received` and, for
    any serialized IT Asset line, that the web app now shows the asset's
    location updated. As a non-Facility/non-admin role, confirm the
    "Gate Pass" quick action and screens are inaccessible.
11. **Link to a Purchase Requisition** — as a Super Admin/IT Admin, from
    within Add Asset tap "Scan PR/PO QR" and scan a real Approved PR's QR
    (printed from its detail page or PDF, per the backend's Extension 4
    Phase 20 deploy); confirm you land back on Add Asset with the PR
    linked, submit, and confirm the new asset's PR link shows correctly
    on the existing web app's PR "Fulfilled By" section. Try a PR with
    several open lines (confirm the picker appears) and one with zero
    open lines (confirm a clear message and that you can still submit
    unlinked).
12. **Rental fields** — from Add Asset, toggle Ownership to Rented, pick
    a vendor, set rental start/end dates, submit, and confirm the new
    asset shows Ownership/Vendor/Rental dates correctly in the existing
    web app's Asset view.
