...
// Software Cost report: total license cost per software title, aggregated across all pools,
// with seat usage for ROI context.
function loadSoftwareCostReport() {
  return action('loadSoftwareCostReport', 'SQL', {
    datasourceName: 'PPS License Asset DB',
    query: `
      SELECT
        s.name AS software_name,
        s.vendor,
        COUNT(DISTINCT li.id) AS pool_count,
...
export default loadSoftwareCostReport;
